...
def gen_section(sectionConfig, flattenedData, sectionKeys, svc, logger):
    ...
    sectionValues = collections.Counter(
        allValues) - collections.Counter(subKeys)
...
def gen_docs(svc, logger):
    try:
        with open(os.path.join("./", svc, "values.yaml"), "r") as stream:
            try:
                ...
                for section in sections:
                    final += gen_section(section, flat,
                                         sectionKeys, svc, logger=logger)
                return final
        ...
        logger.error(
            f'Failed opening values file. Does the service "{svc}" exist?')
        raise err
